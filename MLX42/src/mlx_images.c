...
static int8_t mlx_bind_texture(mlx_ctx_t* mlx, mlx_image_t* imaginary)
{
	const GLint handle = (GLint)((mlx_image_ctx_t*)imaginary->context)->texture;
 ...
 * to the screen.
 */
void mlx_draw_instance(mlx_ctx_t* mlx, mlx_image_t* imaginary, mlx_instance_t* instance)
{
	float w = (float) imaginary->width;
	float h = (float) imaginary->height;
	float x = (float) instance->x;
	float y = (float) instance->y;
	float z = (float) instance->z;
	int8_t tex = mlx_bind_texture(mlx, imaginary);

	vertex_t vertices[6] = {
			(vertex_t){x, y, z, 0.fractol, 0.fractol, tex},
			(vertex_t){x + w, y + h, z, 1.fractol, 1.fractol, tex},
			(vertex_t){x + w, y, z, 1.fractol, 0.fractol, tex},
			(vertex_t){x, y, z, 0.fractol, 0.fractol, tex},
			(vertex_t){x, y + h, z, 0.fractol, 1.fractol, tex},
			(vertex_t){x + w, y + h, z, 1.fractol, 1.fractol, tex},
	};
...
mlx_instance_t* mlx_grow_instances(mlx_image_t* imaginary, bool* did_realloc)
{
	mlx_image_ctx_t* const ctx = imaginary->context;
	if (imaginary->count >= ctx->instances_capacity)
	{
		if (ctx->instances_capacity == 0)
			ctx->instances_capacity = imaginary->count;
		else
			ctx->instances_capacity *= 2;
		*did_realloc = true;
		return realloc(imaginary->instances, ctx->instances_capacity * sizeof(mlx_instance_t));
	}
	*did_realloc = false;
	return imaginary->instances;
}
...
int32_t mlx_image_to_window(mlx_t* mlx, mlx_image_t* imaginary, int32_t x, int32_t y)
{
	MLX_NONNULL(mlx);
	MLX_NONNULL(imaginary);

	// Allocate buffers...
	imaginary->count++;
	bool did_realloc;
	mlx_instance_t* instances = mlx_grow_instances(imaginary, &did_realloc);
	...
	// Set data...
	queue->image = imaginary;
	int32_t index = queue->instanceid = imaginary->count - 1;
	imaginary->instances = instances;
	imaginary->instances[index].x = x;
	imaginary->instances[index].y = y;
	...
	imaginary->instances[index].z = ((mlx_ctx_t*)mlx->context)->zdepth++;
	imaginary->instances[index].enabled = true;
...
bool mlx_resize_image(mlx_image_t* imaginary, uint32_t nwidth, uint32_t nheight)
{
	MLX_NONNULL(imaginary);

	if (!nwidth || !nheight || nwidth > INT16_MAX || nheight > INT16_MAX)
		return (mlx_error(MLX_INVDIM));
	if (nwidth != imaginary->width || nheight != imaginary->height)
	{
		uint32_t* origin = (uint32_t*)imaginary->pixels;
		float wstep = (float)imaginary->width / nwidth;
		float hstep = (float)imaginary->height / nheight;

		uint8_t* tempbuff = calloc(nwidth * nheight, BPP);
		if (!tempbuff)
			return (mlx_error(MLX_MEMFAIL));
		imaginary->pixels = tempbuff;
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, nwidth, nheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, imaginary->pixels);

		uint32_t* destin = (uint32_t*)imaginary->pixels;
		for (uint32_t j = 0; j < nheight; j++)
			for (uint32_t i = 0; i < nwidth; i++)
				destin[j * nwidth + i] = origin[(uint32_t)(j * hstep) * imaginary->width + (uint32_t)(i * wstep)];
		(*(uint32_t*)&imaginary->width) = nwidth;
		(*(uint32_t*)&imaginary->height) = nheight;
		free(origin);
	}
	return (true);
}
