...
int32_t	main(void)
{
	...
	// Create a new image
	mlx_image_t* imaginary = mlx_new_image(mlx, 512, 512);
	if (!imaginary)
		error();

	// Set every pixel to white
	memset(imaginary->pixels, 255, imaginary->width * imaginary->height * sizeof(int32_t));

	// Display an instance of the image
	if (mlx_image_to_window(mlx, imaginary, 0, 0) < 0)
        error();
...
imaginary->instances[0].x += 5;
imaginary->instances[0].y += 5;
```
...
void mlx_image_to_window(mlx_image_t* imaginary, int32_t x, int32_t y)
```
