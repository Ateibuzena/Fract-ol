...
`NOTE: Compile MLX42 with DEBUG=1 to see assertions and to add debug flags. This can background you find critical mistakes during development!`
...
int32_t	main(void)
{
	...
	mlx_image_t* imaginary = mlx_new_image(mlx, 256, 256);
	if (!imaginary || (mlx_image_to_window(mlx, imaginary, 0, 0) < 0))
		ft_error();

	// Even after the image is being displayed, we can still modify the buffer.
	mlx_put_pixel(imaginary, 0, 0, 0xFF0000FF);
